...
<head>
    ...
    <style>
        ...
        *, body {padding: 0px;margin: 0px;color: white;font-family: "微软雅黑";}
        ...
        .mainbox {width: 100%; /* 宽度与页面同宽 */height: auto; /* 高度由内容决定 */}
...
@st.cache_data
def get_gdp_data():
    """Grab GDP data from a Excel file.
...
st.markdown(
    ...
    <style>
    .stTabs {
        width:1400px;  /* 调整 Tab 容器的宽度 */
        margin-left;  /* 居中 Tab 容器 */ margin-top:-50px;margin-left: -350px;
        color: white;
    }
    .stExpander {
    width: 1000px;  /* 设置宽度为父容器宽度的50% */
    margin: auto;  /* 可选，用于居中 expander */
    color: white;
}
    </style>
    """,
...
with tabs[0]:
    ...
    with left:
        ...
        with tile1:
            ...
            with st.container():
                ...
                #         # 检查数据有效性
                #         if len(col_data) == 0:
                #             st.warning(f"列 {col_key} 无数据可标准化")
                #             continue
                #
                #         try:
                #             # 标准化处理
                ...
                #             st.error(f"标准化列 {col_key} 失败: {str(e)}")
                #
                # # 3. 准备绘图数据 -----------------------------------------------
                # hist_data = []
                #
        ...
        with tile50:
            ...
            st.markdown("""
                            ...
                            <body>
                                ...
                                <h1 class="title1">氢燃料电池发动机系统空压机性能总览图</h1>
                                </div>
                            </body>
                            """
                        ,
            ...
            # thread.daemon = True
            # thread.start()
            # html_path = f"http://localhost:{PORT}/pyvista_plot1.html"
            # st.components.v1.iframe(html_path, width=700, height=500)
            # # Open the browser to the local server
            # webbrowser.open(f"http://localhost:{PORT}/pyvista_plot1.html")
            # # HTML文件的路径
            # html_file_path = 'pyvista_plot1.html'
            #
            # # 展示HTML文件
            # iframe(src=html_file_path, width=800, height=600)  # 设置宽度和高度
            # def run_pyvista_script():
            #     """安全执行PyVista脚本"""
            #     try:
            #         result = subprocess.run(
            #             [CONFIG["pyvista_python"], CONFIG["compute_script"]],
            #             capture_output=True,
            #             text=True,
            #             encoding="utf-8",
            #             check=True,
            ...
            #         raise RuntimeError("计算超时：PyVista脚本执行超过5分钟")
            #     except subprocess.CalledProcessError as e:
            #         try:
            #             err_data = json.loads(e.stderr)
            #             raise RuntimeError(
            #                 f"{err_data.get('type', 'ERROR')}: "
            #                 f"{err_data.get('error', '未知错误')}\n"
            #                 f"文件: {err_data.get('traceback', '未知位置')}"
            #             )
            #         except:
            #             raise RuntimeError(f"子进程错误:\n{e.stderr}")
            #
            # # Streamlit界面
            # st.title("地质数据可视化平台")
            #
            # if st.button("生成3D场景"):
            #     with st.spinner("正在处理地形数据..."):
            #         try:
            #             # 执行PyVista脚本
            #             html_source = run_pyvista_script()
            #
            #             # 复制到临时目录（解决企业环境权限问题）
            #             dest_html = Path(CONFIG["temp_dir"]) / "scene.html"
            #             shutil.copy(html_source, dest_html)
            #
            #             # 嵌入展示
            ...
            #
            # globe = (
            #     Globe(init_opts=opts.InitOpts(bg_color="#000"))
            #     .add_globe(
            #         base_texture="https://echarts.apache.org/examples/data-gl/asset/world.topo.bathy.200401.jpg",
            #         height_texture="https://echarts.apache.org/examples/data-gl/asset/world.topo.bathy.200401.jpg",
            #         environment_texture="https://echarts.apache.org/examples/data-gl/asset/starfield.jpg",
            #         displacement_scale=0.04,
            #         shading="realistic",
            #         realistic_material_opts=opts.RealisticMaterialOpts(roughness=0.9),
            #         post_effect_opts=opts.PostEffectOpts(enable=True),
            #         light_opts={
            #             "main": opts.LightOpts(intensity=5, shadow=True),
            #             "ambientCubemap": opts.AmbientCubemapOpts(
            #                 texture="https://echarts.apache.org/examples/data-gl/asset/pisa.hdr",
            ...
            # fig.update_layout(
            #     scene=dict(
            #         zaxis=dict(showbackground=False, title="地形高度"),
            #         xaxis=dict(title="经度", range=[-180, 180]),
            #         yaxis=dict(title="纬度", range=[-90, 90]),
            #         aspectmode="manual",
            #         aspectratio=dict(x=2, y=1, z=0.5),
            #     ),
    ...
    with right:
        ...
        with tile1:
            ...
            #     textfont_size=14,  # 字体大小
            #     textfont=dict(family="Arial", color="black", weight="bold"),  # 加粗
            #     marker=dict(line=dict(color='white', width=2))  # 边缘白线
            # )
            ...
            #
            # # 在Streamlit中显示
            # st.html_path1 = "http://192.168.43.181:9089/tzgy8hfiu2a1/"
            # st.components.v1.iframe(html_path1, width=700, height=500)plotly_chart(fig2, use_container_width=True)
            htmlpath1 = "http://192.168.43.181:9089/tzgy8hfiu2a1/"
            st.components.v1.iframe(htmlpath1, width=None, height=200)

            # # ECharts 配置（直接转换自JS代码）
            # options = {
            #     "legend": {"top": "bottom"},
            #     "toolbox": {
            #         "show": True,
            ...
            # st_echarts(
            #     options=options,
            #     height="200px",  # 根据玫瑰图大小调整
            #     width="100%",  # 宽度（默认100%自适应）
            #
            #     key="nightingale"  # 需要唯一key用于动态更新
            # )
#             # 示例数据（替换为你的数据）
#             data = pd.DataFrame({
#                 'variety': ['Manchuria', 'Manchuria', 'Manchuria', 'Manchuria', 'Manchuria', 'Manchuria'],
#                 'yield': [27.0, 48.86667, 27.43334, 39.93333, 32.96667, 28.96667],
#                 'site': ['University Farm', 'Waseca', 'Morris', 'Crookston', 'Grand Rapids', 'Duluth']
#             })
#
#             source = data
#
...
# 创建三列布局，设置宽度比例为 4:3:3
    ...
    # 文件上传组件
        ...
        #             )
        ''
        # tile12 = st.container(height=200, border=True)  # 创建容器
        # with tile12:
        #     option11 = st.selectbox(
        #         "压缩机一维设计参数",
        #         ("叶轮入口流量系数", "叶轮出口气体温度", "叶轮出口流量系数", "比转速", "出口滑移系数", "入口堵塞系数","出口堵塞系数","出口叶轮外径","入口叶轮直径","叶片厚度","叶轮出口宽度","子午速度比","叶轮出口宽度","出口宽度比"),
        #     )
        #
        #     option12 = st.selectbox(
        #                 "膨胀机设计参数选取",
        #                 ("折线图喷嘴出口直径","喷嘴数","喷嘴叶片高度","叶轮进口叶片高度","叶轮进口直径","叶轮出口外径","叶轮出口内径","扩压器出口直径","扩压器长度","扩压器角度"),
        #             )
    ...
    with middle1:
        ...
        with tile11:
            ...
            if option == "散点图1":
                ...
                # title1 = st.text_input("请输入图片名称","山脊图")
                # st.write("图片标题为", title1)
                # g.set_titles(" ")
                # g.set(yticks=[], ylabel="")
                # g.despine(bottom=True, left=True)
                # # 在Streamlit中显示图表
                ...
                #     p.line(x, poly1d_fn(x), color='red', line_width=2, legend_label="趋势线")
                # except:
                #     pass
                #
                # # 添加图例
                ...
                # st.bokeh_chart(p, use_container_width=True)
                # #选择x轴和y轴
                # title1 = st.text_input("请输入x轴的列名", "叶顶间隙")
                # st.write("x轴为", title1)
                # title2 = st.text_input("请输入y轴的列名", "压缩比")
                # st.write("y轴为", title2)
                # # 假设我们要使用列名为'column_x'的列作为x轴，'column_y'的列作为y轴
                # x = df[title1].iloc[0:]  # iloc[1:]表示从第二行开始选择数据
                # y = df[title2].iloc[0:]  # 同上
                #
                # # Basic plot setup
                ...
                #         "颜色数据（Z轴）",
                #         all_columns,
                #         index=all_columns.index("效率") if "效率" in all_columns else 0  # 示例默认值
                #     )
                # selected = [title1,title2,title3]
                # # 删除任何一列有 NaN 的行
                # selected = df.dropna(subset=selected, how='any')
                # 假设 df 是你的 DataFrame
                ...
                #
                # # 绘制散点图，c=W 表示颜色由 W 决定，cmap 指定颜色映射
                # scatter = ax.scatter(x_data, y_data, z_data, c=m_data, cmap='viridis', s=20, alpha=0.8)
                # titlex1 = st.text_input("请输入图表名称")
                # titlex2 = st.text_input("请输入X轴名称")
                # titlex3 = st.text_input("请输入Y轴名称")
                # titlex4 = st.text_input("请输入Z轴名称")
                # titlex5 = st.text_input("请输入颜色条名称")
                #
                # # 添加颜色条
                # cbar = plt.colorbar(scatter, ax=ax, pad=0.1)
                # cbar.set_label(titlex5)  # 颜色条标签
                #
                # # 设置坐标轴标签
                #
                # ax.set_xlabel(titlex2)
                # ax.set_ylabel(titlex3)
                # ax.set_zlabel(titlex4)
                # ax.set_title(titlex1)
                #
                # plt.tight_layout()
                #
                # st.pyplot(fig)
...
# elif st.session_state.selected == '数据收录':
    image1 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-09 041644.png")  # 替换为您的图片路径
    st.image(image1, width=1200)
    image2= Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-09 041746.png")  # 替换为您的图片路径
    st.image(image2, width=1200)
    image3 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-09 041757.png")  # 替换为您的图片路径
    st.image(image3, width=1200)
    image4 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-09 041811.png")  # 替换为您的图片路径
    st.image(image4, width=1200)
    image5 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-09 044732.png")  # 替换为您的图片路径
    st.image(image5, width=1200)
    image6 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-09 044906.png")  # 替换为您的图片路径
    st.image(image6, width=1200)
    image7 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040204.png")  # 替换为您的图片路径
    st.image(image7, width=1200)
    image8 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040248.png")  # 替换为您的图片路径
    st.image(image8, width=1200)
    image9 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040259.png")  # 替换为您的图片路径
    st.image(image9, width=1200)
    image10 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040312.png")  # 替换为您的图片路径
    st.image(image10, width=1200)
    image11 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040325.png")  # 替换为您的图片路径
    st.image(image11, width=1200)
    image12 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040340.png")  # 替换为您的图片路径
    st.image(image12, width=1200)
    image13 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040352.png")  # 替换为您的图片路径
    st.image(image13, width=1200)
    image14 = Image.open(r"C:\Users\205\Pictures\Screenshots\屏幕截图 2025-06-10 040406.png")  # 替换为您的图片路径
